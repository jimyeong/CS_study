...
class UnionFind{
    private:
    vector<int> _root; // shows connecting information with a node's parent

    public:
    int find(int node){
        // O(n);
        // the subject node needs to traverse through all connectivity to figure out the root of the node(subject);
